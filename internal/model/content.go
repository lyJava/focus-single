...
type ContentListItem struct {
	...
	CategoryId uint        `json:"category_id,omitempty"` // 栏目ID
	UserId     uint        `json:"user_id,omitempty"`     // 用户ID
	Title      string      `json:"title,omitempty"`       // 标题
	Sort       uint        `json:"sort,omitempty"`        // 排序，数值越低越靠前，默认为添加时的时间戳，可用于置顶
	Brief      string      `json:"brief,omitempty"`       // 摘要
	Thumb      string      `json:"thumb,omitempty"`       // 缩略图
	Tags       string      `json:"tags,omitempty"`        // 标签名称列表，以JSON存储
	Referer    string      `json:"referer,omitempty"`     // 内容来源，例如github/gitee
	Status     uint        `json:"status,omitempty"`      // 状态 0: 正常, 1: 禁用
	ViewCount  uint        `json:"view_count,omitempty"`  // 浏览数量
	ReplyCount uint        `json:"reply_count,omitempty"` // 回复数量
